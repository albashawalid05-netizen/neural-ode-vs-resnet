...
\paragraph{Baselines.}
We compare: (i) a GRU baseline; (ii) a LastObsMLP baseline using last observed values; and (iii) GRU-D, which adds learned exponential decay toward feature means driven by time since last observation.

\begin{table}[t]
\centering
\caption{PhysioNet 2012 Set A mortality classification (test). Mean $\pm$ 95\% CI over 3 seeds (0,1,2).}
\label{tab:physio}
\begin{tabular}{lcc}
\toprule
Model & AUROC $\uparrow$ & AUPRC $\uparrow$\\
\midrule
GRU & 0.804 $\pm$ 0.032 & 0.477 $\pm$ 0.059\\
LastObsMLP & 0.817 $\pm$ 0.063 & 0.485 $\pm$ 0.137\\
GRU-D & 0.829 $\pm$ 0.034 & 0.499 $\pm$ 0.112\\
ODE-RNN & 0.709 $\pm$ 0.080 & 0.334 $\pm$ 0.117 \\
\bottomrule
\end{tabular}
\end{table}

On PhysioNet 2012, we include an ODE-based baseline (ODE-RNN) alongside strong discrete-time baselines (GRU, GRU-D). Across 3 seeds (95\% CI), GRU-D performs best, while ODE-RNN is less competitive under this training budget, highlighting that handling missingness explicitly can matter more than continuous-time inductive bias in this setting.


\paragraph{Failure mode.}
We stratify test AUROC by observation-count quartile and compare all three models, highlighting robustness under sparse measurements.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{../results/figures/physio_auroc_by_obs_quartile_models.png}
\caption{Test AUROC stratified by observation-count quartile (mean $\pm$ 95\% CI over seeds), comparing GRU, LastObsMLP, and GRU-D.}
